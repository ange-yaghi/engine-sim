...
void MainMenu::getEngines() {
    std::map< std::pair<std::string, bool>, std::pair<std::string, std::filesystem::path>> engines;
    m_manCount = 0;
    m_man.clear();
            ...
            if (ext == ".mr") {
                ...
                for (int e = 0; e < i; e++) {
                    if (lines[e]._Starts_with("public node")) {
                        if (lines[e]._Starts_with("public node main")) {
                            hasMain = true;
                        }
                        else if (lines[e + 1].find("alias output __out: engine;") != std::string::npos) {
                            // engine node thing
                ...
                engines.insert(std::make_pair(std::make_pair(nodeName, hasMain), std::make_pair(entry.path().parent_path().generic_string().replace(0, 18, ""), entry.path())));
                file.close();
            }
        }
        else {
        ...
        if (var.second.first == m_man[m_selectedMan]) {
            if (i == index) {
                ...
                if (var.first.second) {
                    ...
                    file << "set_engine(" << std::endl;
                    file << "    " << var.first.first << "()" << std::endl;
                    file << ")" << std::endl;
                }
        ...
        for each (auto var in m_engines) {
            if (var.second.first == m_man[m_selectedMan]) {
                if (i >= start) {
                    ...
                    std::string text1 = var.first.first;
